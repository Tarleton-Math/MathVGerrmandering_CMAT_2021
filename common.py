...
class Gerry:
    ...
    def file_id(self, variable, yr=2020, suffix='parquet'):
        tbl = self.table_id(variable, yr)
        f = tbl.split('.')[-1]
        return data_path / f"{f.replace('_','/')}/{f}.{suffix}"
    
    def load_table(self, variable, yr=2020, tbl=None, df=None, file=None, query=None, overwrite=False, preview_rows=0):
        if tbl is None:
            tbl = self.table_id(variable, yr)
        if overwrite:
            bqclient.delete_table(tbl, not_found_ok=True)
        if df is not None:
            job = bqclient.load_table_from_dataframe(df, tbl)
            job.result()
            df.to_parquet(self.file_id(variable, yr))
        elif file is not None:
            with open(file, mode="rb") as f:
                job = bqclient.load_table_from_file(f, tbl, job_config=bigquery.LoadJobConfig(autodetect=True))
                job.result()
        elif query is not None:
            job = bqclient.query(query, job_config=bigquery.QueryJobConfig(destination=tbl))
            job.result()
        else:
            ...
            for fn in zipfile.namelist():
                if fn[-3:] == '.pl':
                    ...
                    if fn[2:5] == 'geo':
                        geo_tbl  = tbl + 'geo'
                        temp_tbl = tbl + 'temp'
                        self.load_table(variable, yr, file=file, overwrite=overwrite, tbl=temp_tbl)
                        ...
                        query = 'select\n\t' + ',\n\t'.join(sel) + '\nfrom\n\t' + temp_tbl
                        self.load_table(variable, yr, query=query, overwrite=overwrite, tbl=geo_tbl)
                        ...
                        if i in ['1', '2']:
                            ...
                            os.system(cmd)
                            self.load_table(variable, yr, file=file, overwrite=overwrite, tbl=tbl+i)
...
from
    {self.table_id(variable, yr)}_1 as A
inner join
    {self.table_id(variable, yr)}_2 as B
on
    A.fileid = B.fileid
    and A.stusab = B.stusab
    and A.logrecno = B.logrecno
inner join
    {self.table_id(variable, yr)}_geo as C
on
...
from (
    ...
    """
            self.load_table(variable, yr, query=query, overwrite=overwrite)        
    ...
    def get_shapes(self, level='tabblock', yr=2020, overwrite=False):
        ...
        if not overwrite and head(tbl) is not None:
            ...
            while True:
                ...
                df['geometry_simple'] = df['geometry'].simplify(0.003)
                df['centroid'] = gpd.points_from_xy(df[f'intptlon'], df['intptlat'], crs=crs_map)
                df['perim'] = df.to_crs(crs_length).length
                df['polsby_popper'] = 4 * np.pi * df['aland'] / (df['perim']**2)
                df = df[['geoid', 'aland', 'perim', 'polsby_popper', 'centroid', 'geometry', 'geometry_simple']].rename(columns={'geoid':f'geoid_{yr}'})
                ovr = a==0
                self.load_table(variable, yr, df=df.to_wkb(), overwrite=ovr)
                if df.shape[0] < self.chunk_size:
                    break
                else:
                    a += self.chunk_size

    def read_shapes(self, level='tabblock', yr=2020, start=None, rows=None):
        tbl = self.table_id(f'shapes_{level}', yr)
        df = read_table(tbl, start, rows)
    ...
    def get_pairs(self, level='tabblock', yr=2020, overwrite=False):
        ...
        if not overwrite and head(tbl) is not None:
            ...
            cols = [f'geoid_{yr}', 'geometry', 'centroid']
            a1 = 0
            while True:
                df1 = self.read_shape(level, yr, a1, self.chunk_size)[cols].to_crs(crs_length)
                a2 = a1 + self.chunk_size
                while True:
                    df2 = self.read_shape(level, yr, a2, self.chunk_size)[cols].to_crs(crs_length)
                    pairs = df1.merge(df2, how='cross')
                    ...
                    pairs = pairs.drop(columns=[c+z for c in cols[1:] for z in ['_x', '_y']])
                    ovr = (a1==0) and (a2==0)
                    self.load_table(variable, yr, df=pairs, overwrite=ovr)
                    print(a1, a2, pairs['perim_shared'].max())
                    if df2.shape[0] < self.chunk_size:
                        break
                    else:
                        a2 += self.chunk_size
                if df1.shape[0] < self.chunk_size:
                    break
                else:
                    a1 += self.chunk_size
