    ...
    """
        ...
        if self.level != 'tabblock':
            ...
            if abbr == 'TX':
                query = f"""
select
    substring({geoid}, 1, {g}) as {geoid},
    party,
    min({cntyvtd}) as {cntyvtd},
    min(cd) as cd,
    min(sldu) as sldu,
    min(sldl) as sldl,
    sum(votes) as votes,
...
select
    substring({geoid}, 1, {g}) as {geoid},
    --party,
    min({cntyvtd}) as cntyvtd_{cntyvtd},
    min(cd) as cd,
    min(sldu) as sldu,
    min(sldl) as sldl,
    --sum(votes) as votes,
...
on
    E.{geoid} = F.{geoid}
    """
        N = bqclient.query(query).result().to_dataframe().drop(columns='cntyvtd_pop_prop', errors='ignore')
        if abbr == 'TX':
            ...
            N = N.pivot_table(index=i, columns='party', values='votes', aggfunc=np.sum, fill_value=0).reset_index().reset_index(drop=True)
        self.nodes = downcast(N, exclude=[geoid, cntyvtd]).sort_values(geoid)
        print('success', end='')
        load_table(tbl, df=self.nodes, preview_rows=5)
...
#         points = gpd.geoSeries(read_table(self.table_id('shapes', self.level, self.shapes_yr), cols=[g, point])
        shapes_tbl = self.table_id('shapes', self.level, self.shapes_yr)
        for dist, nodes in self.nodes.groupby('district')[g]:
            while True:
                ...
                if len(components) == 1:
                    break
                print(f'District {dist} has {len(components)} connected components with {[len(c) for c in components]} nodes ... adding edges to connect')
...
from (
    ...
    from (
        ...
        from
            ...
            {shapes_tbl} as B
        where
            (A.{g} in ('{c[0]}') and B.{g} in ('{c[1]}')) or
            (A.{g} in ('{c[1]}') and B.{g} in ('{c[0]}'))
        )
    )
...
#                 df = self.read_table(self.table_id('shapes', self.level, self.shapes_yr)).rename(columns={f'geoid_{self.shapes_yr}':'geoid'})
                
#                 sh = [df.query(f'geoid in {c}') for c in components[]]
                
                
                
#                 print(components)
#                 print(f'District {dist} has {len(components)} connected components ... adding edges to connect')
#                 query = f'(geoid_x in {components[0]} & geoid_y in {components[1]}) | (geoid_y in {components[0]} & geoid_x in {components[1]})'
#                 cut_edges = self.edges.query(query)
#                 display(cut_edges)
#                 new_edges = cut_edges.query(f'distance == {cut_edges["distance"].min()}')
#                 display(new_edges)
#                 self.graph.update(self.edges_to_graph(new_edges))
