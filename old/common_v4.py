...
select
    ...
    C.party,
    C.votes * A.cntyvtd_pop_prop as votes
from
    {self.table_id('census', 'tabblock', self.census_yr)} as A
inner join
...
on
    A.cntyvtd_{self.shapes_yr} = C.cntyvtd_{self.shapes_yr}
    """
...
on
    ...
    """
        ...
        N = N.pivot(index=i, columns='party', values='votes').drop(columns=np.nan).reset_index().fillna(0)
        self.node = N
        display(N.head(3))
        load_table(tbl, df=N, preview_rows=5)
        print('success', end='')
