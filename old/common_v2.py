...
#     geo_simplification: float = 0.003
    ...
    def __post_init__(self):
        ...
        assert 2010 <= self['census_yr'] and self['census_yr'] < 2021, f"census_yr must be between 2010 and 2020, got {self['census_yr']}"
        self['geoid_yr'] = 2010 if self['census_yr'] < 2020 else 2020
    ...
    def table_id(self, variable, yr=None):
        yr = self['census_yr'] if yr is None else yr
        return f"{bq_dataset}.{variable}_{yr}_{self['abbr']}"

    def file_id(self, variable, yr=None, suffix='zip'):
        yr = self['census_yr'] if yr is None else yr
        tbl = self.table_id(variable, yr)
        f = tbl.split('.')[-1]
        return data_path / f"{f.replace('_','/')}/{f}.{suffix}"

    def get_assignments(self, yr=None, overwrite=False):
        yr = self['census_yr'] if yr is None else yr
    ...
    def get_crosswalks(self, yr=None, overwrite=False):
        yr = self['geoid_yr'] if yr is None else yr
        variable = 'crosswalks'
        tbl = self.table_id(variable, yr)
        if not overwrite and head(tbl) is not None:
            print('BigQuery table exists', end='')
    ...
    def get_elections(self, yr=None, overwrite=False):
        yr = 2020 if yr is None else yr
        if self['abbr'] != 'TX':
    ...
    def get_census(self, yr=None, overwrite=False):
        yr = self['geoid_yr'] if yr is None else yr
        variable = 'census'
        tbl = self.table_id(variable, yr)
        if not overwrite and head(tbl) is not None:
...
inner join (
    ...
    from (
        select
            ...
            , sum(cast(arealand_int as int)) over (partition by geoid_2010) as area_2010
        from
...
from (
    ...
    def get_shapes(self, level='tabblock', yr=None, overwrite=False):
        yr = self['geoid_yr'] if yr is None else yr
...
order by
    ...
    def get_pairs(self, level='tabblock', yr=None, overwrite=False):
        yr = self['geoid_yr'] if yr is None else yr
...
#             print(query)
    ...
    def get_data(self, overwrite=list()):
        ...
        yr = 2020
        print(f"\nGet {self['name']} {variable.ljust(15, ' ')} {yr}", end=concat_str)
        self[variable] = self.get_shapes(self['level'], yr, variable in overwrite)

        variable = f"pairs_{self['level']}"
        yr = self['geoid_yr']
        print(f"\nGet {self['name']} {variable.ljust(15, ' ')} {yr}", end=concat_str)
        self[variable] = self.get_shapes(self['level'], yr, variable in overwrite)

    def make_graph(self):
        edges = read_table(self.table_id(f"pairs_{self['level']}"))
        display(edges.head(5))
        return nx.from_pandas_edgelist(edges, source='geoid_2020_x', target='geoid_2020_y', edge_attr=['distance', 'shared_perim'])
