...
# class Crosswalks(Variable):
#     name    : str = 'crosswalks'
#     var_lev : typing.Any = Crosswalks_Level
...
#         if not exists['tbl_level']:
#             if exists['df'] or exists['pq']:
#                 load_table(tbl=self.targets()['tbl'], df=self.df, preview_rows=0)
#             else:
#                 self.get_zip()
#                 self.zipfile = zf.ZipFile(self.targets()['zip'])
#                 print(f'creating {level} table', end=concat_str)
#                 self.get_raw()
#         self.cols = self.get_cols(self.targets()['tbl'])
#         print(f'success')
...
#         df['aland_prop'] = (df['arealand_int'] / df['A']).fillna(0)
#         self.df = df[ids+['aland_prop']].sort_values(ids[1])
#         self.df.to_parquet(self.targets()['pq'])
#         load_table(tbl=self.targets()['tbl'], df=self.df, preview_rows=0)