...
class Analysis(Base):
    ...
    def plot(self, show=True):
        rpt(f'map creation for {self.seed}')
        try:
    ...
    #         geo = gpd.GeoSeries.from_wkt(df['polygon'], crs='EPSG:4326').buffer(0) # <-------------------- to not simplify at all
            ...
            fig = self.gdf.plot_bokeh(
                ...
                colormap = "Category20",
                hovertool_columns = ['geoid', 'county', 'total_pop', 'aland'],
    ...
    def get_results(self):
        ...
        try:
            ...
            cols = [c for c in get_cols(self.nodes) if c not in Levels + District_types + ['polygon', 'aland', 'perim', 'polsby_popper', 'point']]
            query = f"""
select
    D.*,
