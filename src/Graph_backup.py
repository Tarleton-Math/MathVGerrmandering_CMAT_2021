...
class Graph(Variable):
    ...
    def get(self):
        print(f"Get {self.name} {self.state.abbr} {self.yr} {self.level} {self.district_type}".ljust(32, ' '), end=concat_str)
        try:
            self.graph
    ...
    def process(self):
        ...
        nx.set_node_attributes(self.graph, self.nodes[['total_pop']].to_dict('index'))
...
#         R = rng.permutation([(a,b) for a in D.keys for b in D.keys if a < b]).tolist()
        ...
        R = rng.permutation([(d0, d1, n0, n1) for d0, n0 in districts.tuple for d1, n1 in districts.tuple if d0 < d1])
        for do, d1, n0, n1 in R:
            N = self.nodes.query(f'{districts.name} in {pair}').copy()
