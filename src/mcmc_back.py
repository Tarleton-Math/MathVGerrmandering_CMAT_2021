...
#         print('MCMC done')
    ...
    def recomb(self):
        self.get_stats()
        L = M.stat['total_pop']
        Z = pd.DataFrame([(x, y, abs(p-q)) for x, p in L.iteritems() for y, q in L.iteritems() if x < y]).set_index([0,1])
...
#             pairs = self.rng.permutation(df.index)
            def gen():
                a = self.rng.choice(Z.index)
...
#             pairs = [(d0, d1) for d0 in L[:k] for d1 in L[k:][::-1]]
            ...
            def gen():
                Z['p'] /= Z['p'].sum()
                a = self.rng.choice(Z.index, p=Z['p'])
                Z.drop(a, inplace=True)
                yield a
            pairs = gen()
